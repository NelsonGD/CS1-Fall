...
{
    float minUtilization = 100.0; // Start at max in case every garaga is full
    int minUtilizationIndx = -1; // Will keep track for the garage index that has the least utilization
    int g = campus->total_garages; // Number of garages to loop over

    // print each garage's utilization report, and do math for utilization
    for(int i = 0; i < g; i++)
    {   
        // calculating utilization from current garage
        ...
        // update minUtilization
        if(utilization < minUtilization)
        {
            minUtilization = utilization;
            minUtilizationIndx = i; // Keep track of index
        }
    }

    // Print least utilized garage
    if(minUtilizationIndx != -1) // Basic check in case index wasn't found
    {
        printf("Least Utilized: %s\n", campus->garages[minUtilizationIndx]->garage_name);
    }
}
...
{
    Garage* newGarage = malloc(sizeof(Garage));
    if(newGarage == NULL)
    {
        printf("newGarage of name, %s, wasn't properly allocated\n", name);
        return NULL;
    }
    ...
    // Malloc for parked vehicles
    newGarage->parked_vehicles = malloc(sizeof(RegisteredVehicle*) * capacity);
    if(newGarage -> parked_vehicles == NULL)
    {
        printf("Parked Vehicles for garage %s wasn't allocated properly\n", name);
        free(newGarage); // No need for newGarage if it's content wasn't loaded properly
        return NULL;
    }

    // malloc for newGarage name
    newGarage->garage_name = malloc(sizeof(char) * strlen(name) + 1);
    if(newGarage->garage_name == NULL)
    {
        printf("New garage, %s, name wasn't allocated properly\n", name);
        free(newGarage -> parked_vehicles); // free due to failure to malloc
        free(newGarage);
        return NULL;
    }
    strcpy(newGarage->garage_name, name);

    // Initialize current count for new garage
...
{
    // Allocate memory for the vehicle structure
    RegisteredVehicle* newVehicle = malloc(sizeof(RegisteredVehicle));
    if (newVehicle == NULL) {
        printf("Error: Failed to allocate memory for vehicle structure with license plate: %s\n", license);
        return NULL;
    }

    // Allocate memory for the license plate string
    newVehicle->license_plate = malloc(sizeof(char) * (strlen(license) + 1));
    if (newVehicle->license_plate == NULL) {
        printf("Error: Failed to allocate memory for license plate: %s\n", license);
        free(newVehicle); // Free previously allocated memory for the vehicle structure
        return NULL;
    }
    strcpy(newVehicle->license_plate, license); // Copy the license plate string

    // Allocate memory for the owner's name string
    newVehicle->owner_name = malloc(sizeof(char) * (strlen(owner) + 1));
    if (newVehicle->owner_name == NULL) {
        printf("Error: Failed to allocate memory for owner name: %s\n", owner);
        free(newVehicle->license_plate); // Free previously allocated memory for license plate
        free(newVehicle); // Free previously allocated memory for vehicle structure
        return NULL;
    }
    strcpy(newVehicle->owner_name, owner); // Copy the owner name

    return newVehicle; // Return the initialized vehicle
}
...
void registerVehicle(Campus* campus, const char* license, const char* owner)
{
    ...
    // Allocate memory for the new vehicle (using a temporary pointer for safety)
    RegisteredVehicle** temp = realloc(campus->registered_vehicles, sizeof(RegisteredVehicle*) * (campus->total_registered_vehicles + 1));
    if (temp == NULL) {
        printf("Campus Registered Vehicles wasn't reallocated for new vehicle properly.\n");
        return;
    }
    campus->registered_vehicles = temp;

    // Create and register the new vehicle
    campus->registered_vehicles[campus->total_registered_vehicles] = createRegisteredVehicle(license, owner);
    if (campus->registered_vehicles[campus->total_registered_vehicles] == NULL) {
        printf("Campus Registered Vehicle for owner %s, plate %s, wasn't properly allocated\n", owner, license);
        return;
    }

    // Increase the count of registered vehicles
...
// Loop through garages looking for vehicle. After it shifts all vehicles
// to fill in the empty spot from the removed car.
int removeVehicleFromGarage(Garage* garage, const char* license)
{
    for(int i = 0; i < garage->current_count; i++)
    {
        ...
        {
            // Remove from parked vehicle list
            for(int j = i; j < garage->current_count - 1; j++)
            {
                garage->parked_vehicles[j] = garage->parked_vehicles[j+1];
            }

            garage->current_count--; // update current count for garage
            return 1; // Car successfully removed
        }
    }
    return 0;
}
...
int  resizeGarage(Garage*  garage,  int  new_capacity)
{
    ...
    // reallocating the garage for teh new capacity
    RegisteredVehicle** newParkedVehicles = realloc(garage->parked_vehicles, sizeof(RegisteredVehicle*) * new_capacity);
    if(newParkedVehicles == NULL)
    {
        printf("Resizing for new vehicles failed.\n");
        return 0;
    }

    // Update values for vehicle array and capacity
    garage->parked_vehicles = newParkedVehicles;
...
{
    int targetGarageIndx = -1; // Index of the target garage
    int foundVehicleInGarage = -1; // Index of the garage where the vehicle is found
    RegisteredVehicle* vehicleToMove = NULL;

    // Find the target garage by its name first
    for (int i = 0; i < campus->total_garages; i++) {
    ...
    // If the target garage is not found
    if (targetGarageIndx == -1) {
        printf("%s NOT FOUND.\n", target_garage_name);
        return 0;
    }

    // Now, search for the vehicle across all garages
    for (int i = 0; i < campus->total_garages; i++) {
        for (int j = 0; j < campus->garages[i]->current_count; j++) {
            if (strcmp(campus->garages[i]->parked_vehicles[j]->license_plate, license) == 0) {
                vehicleToMove = campus->garages[i]->parked_vehicles[j];
                foundVehicleInGarage = i;  // Save the index of the garage where the vehicle is parked
                break;
            }
        }
        if (vehicleToMove != NULL) {
            break; // Stop searching once the vehicle is found
        }
    }

    // If the vehicle is not found on campus
    if (vehicleToMove == NULL) {
    ...
    if (campus->garages[targetGarageIndx]->current_count == campus->garages[targetGarageIndx]->total_capacity) {
        printf("%s IS FULL.\n", target_garage_name);
        return 0;
    }

    // If the vehicle is found in a garage, remove it from that garage
    if (foundVehicleInGarage != -1) {
        removeVehicleFromGarage(campus->garages[foundVehicleInGarage], license);
    }

    // Park the vehicle in the new garage
    parkVehicle(campus->garages[targetGarageIndx], vehicleToMove);
    ...
    */
    ...
    for(int i = 0; i < c; i++)
    {
        ...
        if (strcmp(command, "PARK") == 0) {
            char licensePlate[MAXGSTRINGLEN];
            char gName[MAXGSTRINGLEN];
            scanf("%s %s", licensePlate, gName);
            int garageIndex = 0;

            // Iterate through garage names and update index until garage for parking is found
            ...
            int vehicleIndex = findRegisteredVehicleIndex(campus, licensePlate);
            if (vehicleIndex == -1) {
            ...
            parkVehicle(campus->garages[garageIndex], campus->registered_vehicles[vehicleIndex]);
        }
        // logic for utilization report
        ...
        // command for registering a new vehicle
        ...
        // command for removing vehicle from garage
        else if(strcmp(command, "REMOVE_VEHICLE_GARAGE") == 0)
        {   
            // Take input for plate for car to remove
            char plateVehicleToRemove[MAXGSTRINGLEN];
            scanf("%s", plateVehicleToRemove);
            
            // keep track of indexes for the garage where the vehicle is and where is parked
            int garageIndex = -1; 

            // loop through each garage and look through parked vehicles
            for(int i = 0; i < campus->total_garages; i++)
            {
                for(int j = 0; j < campus->garages[i]->current_count; j++)
                {
                    ...
                    {
                        garageIndex = i;
                        break; // Exit loop as soon as car is found
                    }
                }
                if(garageIndex != -1) // not found
                    break; 
            }
            
            // Only done if vehicle found
            if(garageIndex != -1)
            {
                int removeCarConfirmation = removeVehicleFromGarage(campus->garages[garageIndex], plateVehicleToRemove);
                if(removeCarConfirmation)
                {
        ...
        else if(strcmp(command, "RESIZE") == 0)
        {
            ...
            if(garageIndex != -1)
            {
                resizeGarage(campus->garages[garageIndex], newCapacity);
            }
        }
        ...
        // {
        //     char ownerName[MAXGSTRINGLEN];
        //     scanf("%s", ownerName);
        //     displayVehiclesByOwner(campus, ownerName);
        // }
    ...
    freeMem(campus, campus->total_garages, campus->total_registered_vehicles);
